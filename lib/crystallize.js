...
function safeJsonParse(text) {
  try {
    ...
    if (match) {
      try { return JSON.parse(match[1].trim()); } catch (e2) { /* fall through */ }
    }
    return null;
  }
}
...
async function checkPrincipleAlignment(vectors, config) {
  ...
  for (const vector of vectors) {
    const prompt = [
      ...
      `Existing principles: ${principles.join(', ')}`,
      'If the vector does not fit any existing principle, use principle: "new" and propose a new principle name in the "newPrincipleName" field.',
      'Return strict JSON with keys: principle, confidence, rationale, newPrincipleName (optional).',
      `Vector: ${vector.text}`
    ].join('\n');
    ...
    // Accept known principles OR "new" for novel discoveries
    if (principles.includes(principle) || principle === 'new') {
      results.push({
        vectorId: vector.id,
        principle: principle === 'new' ? (parsed.newPrincipleName || 'new').toLowerCase() : principle,
        isNew: principle === 'new',
